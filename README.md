...
> [!IMPORTANT]
> 默认使用 `safe` 安全同步模式：书籍页面会保留，微信读书自动同步内容会放在书籍下的“微信读书同步内容”子页面中，页面里的个人笔记不会被同步覆盖。
>
...
| 模式 | 行为 | 建议 |
| --- | --- | --- |
| `safe`（默认） | 更新书籍属性，并只重建“微信读书同步内容”子页面 | 日常使用 |
| `replace` | 删除原书籍页面后重新创建 | 兼容旧模板或需要重置数据时使用 |

安全模式第一次运行时，旧版本已经生成的书籍内容会保留，同时创建新的同步子页面；确认新页面内容正确后，可以手动整理旧内容。
